...
export default function AcceptInvite() {
  ...
  const [invite, setInvite]     = useState<{ email: string; name: string; org_name: string } | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`/api/invites?token=${encodeURIComponent(token)}`)
      ...
      .then((data) => {
        ...
        else setTokenError(data.error || "Invalid or expired invite link.");
      })
      .catch(() => setTokenError("Failed to load invite."))
            ...
            <AnimatePresence mode="wait">
              {success ? (
                ...
                >
                  ...
                  <p className="text-sm text-muted-foreground mb-1">Account created successfully.</p>
                ...
                <motion.div key="form">
                  ...
                  <div className="mb-10">
                    ...
                    >
                      Welcome,<br />
                      <span className="text-primary">{invite?.name?.split(" ")[0]}.</span>
                    </h1>
                    <p className="mt-4 text-sm text-muted-foreground leading-relaxed">
                      Set a password for <span className="text-foreground font-medium">{invite?.email}</span> to activate your account.
                    </p>
                  </div>
                  ...
                  <p className="mt-6 font-mono text-[10px] text-muted-foreground tracking-widest uppercase text-center">
