...
export async function handleCreateInvite(req: any, res: any) {
  ...
  try {
    ...
    await pool.query(
      ...
      [token, email, name, role, targetOrgId, user.id, expiresAt.toISOString()]
    );
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO users (email, name, password_hash, role, organisation_id)
       VALUES ($1, $2, $3, $4, $5)`,
      [invite.email, invite.name, passwordHash, invite.role, invite.organisation_id]
    );
...
export async function handleResendInvite(req: any, res: any) {
  ...
  try {
    ...
    await pool.query(
      ...
      [newToken, newExpiry.toISOString(), invite_id]
    );
...
export async function handleCancelInvite(req: any, res: any) {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT organisation_id FROM invites WHERE id = $1`,
      [invite_id]
    );
    if (rows.length === 0) return res.status(404).json({ error: 'Invite not found' });
    ...
    await pool.query(`DELETE FROM invites WHERE id = $1`, [invite_id]);
      ...
      <tr><td style="background-color:#FBEFD9;border:1px solid #D4B896;padding:52px 48px;">
        <table role="presentation" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
          <tr>
            ...
            <td style="padding-left:12px;vertical-align:middle;font-family:monospace,'Courier New';font-size:10px;letter-spacing:0.18em;text-transform:uppercase;color:#C97B3A;">Client Portal &middot; Invitation</td>
          </tr>
        </table>
        <h1 style="font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;font-size:44px;font-weight:900;color:#1A1008;margin:0 0 4px 0;line-height:1.05;letter-spacing:-0.025em;">You're<br>invited.</h1>
        ...
        <p style="font-size:15px;line-height:1.7;color:#3D2810;margin:0 0 36px 0;"><strong>${inviterName}</strong> has invited you to access the <strong>${orgName}</strong> client portal on DevEdge. Click below to set your password and get started.</p>
        <table role="presentation" cellpadding="0" cellspacing="0" style="margin-bottom:36px;">
          <tr>
            <td style="background-color:#C97B3A;">
              <a href="${acceptUrl}" style="display:inline-block;padding:15px 36px;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;font-size:13px;font-weight:700;color:#ffffff;text-decoration:none;letter-spacing:0.07em;text-transform:uppercase;">Accept Invitation &rarr;</a>
            </td>
          </tr>
        </table>
        ...
        <p style="font-family:monospace,'Courier New';font-size:10px;color:#7A5C3A;margin:0 0 8px 0;letter-spacing:0.03em;">This link expires in 48 hours. If you weren't expecting this, you can safely ignore it.</p>
